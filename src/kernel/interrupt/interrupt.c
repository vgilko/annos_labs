...
void interrupt_init(void) {
    ...
    idt[INTERRUPT_VECTOR_DEVICE_NOT_AVAILABLE] = INTERRUPT_GATE(GD_KT, interrupt_handler_device_not_available, 0,
                                                                IDT_DPL_S);
    ...
    idt[INTERRUPT_VECTOR_SEGMENT_NOT_PRESENT] = INTERRUPT_GATE(GD_KT, interrupt_handler_segment_not_present, 0,
                                                               IDT_DPL_S);
    idt[INTERRUPT_VECTOR_STACK] = INTERRUPT_GATE(GD_KT, interrupt_handler_stack, 0, IDT_DPL_S);
    idt[INTERRUPT_VECTOR_GENERAL_PROTECTION] = INTERRUPT_GATE(GD_KT, interrupt_handler_general_protection, 0,
                                                              IDT_DPL_S);
    idt[INTERRUPT_VECTOR_PAGE_FAULT] = INTERRUPT_GATE(GD_KT, interrupt_handler_page_fault, 0, IDT_DPL_S);
    idt[INTERRUPT_VECTOR_X86_FP_INSTRUCTION] = INTERRUPT_GATE(GD_KT, interrupt_handler_x86_floating_point_instruction,
                                                              0, IDT_DPL_S);
    ...
    idt[INTERRUPT_VECTOR_SECURITY_EXCEPTION] = INTERRUPT_GATE(GD_KT, interrupt_handler_security_exception, 0,
                                                              IDT_DPL_S);
    ...
    // stack (use cpu->pml4)
    for (uintptr_t stack_addr = INTERRUPT_STACK_TOP - INTERRUPT_STACK_SIZE;
         stack_addr < INTERRUPT_STACK_TOP; stack_addr += PAGE_SIZE) {
        struct page *page;
    ...
    for (uintptr_t stack_addr = EXCEPTION_STACK_TOP - EXCEPTION_STACK_SIZE;
         stack_addr < EXCEPTION_STACK_TOP; stack_addr += PAGE_SIZE) {
        struct page *page;
